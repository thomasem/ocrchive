...
def find_receipt_contour(edged: cv2.typing.MatLike) -> cv2.typing.MatLike:
    '''Find major contour (receipt) and approximate polygon to four sides'''
    contours, _ = cv2.findContours(edged, cv2.RETR_EXTERNAL,
        cv2.CHAIN_APPROX_SIMPLE)

    if len(contours) < 1:
        raise ValueError('No major contours')
    if len(contours) > 1:
        raise ValueError('More than one major contour')

    # Sort largest to smallest
    contours = sorted(contours, key=cv2.contourArea, reverse=True)

    # Use largest contour to compute the convex hull to work from
    ...
    return numpy.array([topmost, rightmost, bottommost, leftmost])
...
if __name__ == '__main__':
    ...
    contour = find_receipt_contour(edge_detection(resized))

    # Need to also add color channels back
    traced = cv2.cvtColor(resized.copy(), cv2.COLOR_GRAY2BGR)
    cv2.drawContours(traced, [contour], -1, (0,255,255), 2)
    show('Traced', traced)
